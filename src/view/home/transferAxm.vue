<template>
  <div class="mine_box">
    <my-header :showBack="true" title="划转" :showRight="true" />
    <div class="content">
      <!-- <div class="titles">
      ...
      <div class="mode">
        <p>提币方式</p>
        <div class="mode_bg">
          <van-button type="primary" size="large">AXM</van-button>
          <van-button type="primary" size="large" @click="pushPath('/transfer')">USDT</van-button>
        </div>
      </div>
      <ul class="extract">
        <li>
        ...
        <li>
          <p>数量</p>
          <input type="text" maxlength="11" v-model="num" placeholder="划转数量">
          <span>全部</span>
          <span>可用AXM: {{sweets.sweets}}</span>
        </li>
        <li v-if="flag">
          <p>手续费</p>
          ...
          <span style="font-size: 3.7vw;font-weight: bold">{{sweets.serviceFee*100+'%'}}</span>
          <span></span>
        </li>
      </ul>
      <div class="main_bgs">
      ...
      <div class="password">
        <van-popup v-model="show"  position="bottom" :style="{ height: '62%' }">
          ...
          />
          <p class="del">
            <span>扣除金额:&nbsp;&nbsp;{{parseInt(num)+num*sweets.serviceFee}}&nbsp;&nbsp;AXM</span> 
          </p>
          <!-- 数字键盘 -->
          <van-number-keyboard
            :show="showKeyboard"
  ...
  methods: {
    ...
    onInput(key) {
      ...
      if (this.value.length === 6) {
        // this.errorInfo = '密码错误';
        let params = {toAccount:this.toAccount,num:this.num,payPwd:this.value};
        var formObject = encodeParams(params, this.userInfo.userId, this.userInfo.tokenId, this.userInfo.appKey);
        this.$ajax(this.$api.transferCoin, {toAccount:this.toAccount,num:this.num,payPwd:this.value},formObject.sign).then(res => {
          if (res.code == 200) {
    ...
    getAXM(){
      Toast({ message:"抱歉，暂时未开放此功能!", position: "center",duration:"1000"  });
    },
    getCustInfo() {
      let params = {};
      var formObject = encodeParams(params, this.userInfo.userId, this.userInfo.tokenId, this.userInfo.appKey);
